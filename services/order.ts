...
export const orderService = {
  ...
  paymentList: () => {
    ...
    return fetcher<DefaultResponse<Payment[]>>(buildUrlQueryParams("v1/rest/payments", params));
  },
  deliveryPoints: (params: ParamsType) => {
    const lang = getSupportedLang();
    const finalParams = lang ? { lang, ...params } : params;
    return fetcher<Paginate<DeliveryPoint>>(
      buildUrlQueryParams("v1/rest/delivery-points", finalParams)
    );
  },
  ...
  checkCoupon: (params?: ParamsType) => {
    const lang = getSupportedLang();
    const finalParams = lang ? { lang, ...params } : params;
    return fetcher.post<DefaultResponse<Coupon>>(
      buildUrlQueryParams("v1/rest/coupons/check", finalParams)
    );
  },
  getAll: (params?: ParamsType) => {
    const lang = getSupportedLang();
    const finalParams = lang ? { lang, ...params } : params;
    return fetcher<Paginate<Order>>(
      buildUrlQueryParams("v1/dashboard/user/orders/paginate", finalParams)
    );
  },
  get: (id?: number | null, params?: ParamsType) => {
    const lang = getSupportedLang();
    const finalParams = lang ? { lang, ...params } : params;
    return fetcher<DefaultResponse<OrderFull[]>>(
      buildUrlQueryParams(`v1/dashboard/user/orders/${id}/get-all`, finalParams),
      {
